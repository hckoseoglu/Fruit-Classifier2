...
# Train time (kNN "training" is basically storing data)
t0 = time.perf_counter()
model.fit(X_train_s, y_train)
train_time_sec = time.perf_counter() - t0
print(f"kNN fit time: {train_time_sec:.6f} s")

# Inference time (kNN is expensive here)
t0 = time.perf_counter()
...
print(f"Inference time (test): {infer_time_sec:.4f} s")
