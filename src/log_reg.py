...
EXTRACTED_DATASET_FILE = "./dataset_ai_extracted_reduced.json"
...
# Load processors (to get label encoder)
...
if USE_PCA:
    ...
    k_optimal, errors = explore_intrinsic_dimensionality(
        X_train_s, variance_threshold=0.90, plot=False
    )
    print(f"Optimal PCA components to retain 90% variance: {k_optimal}")
...
print(f"Logistic Regression training time: {train_time_sec:.4f} s")
...
train_accuracy = model.score(X_train_s, y_train)
