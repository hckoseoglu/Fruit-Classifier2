...
text_feats = np.array([e["text_features"] for e in data])
numeric_feats = np.array([e["numerical_features"] for e in data])
cat_feats = np.array([e["categorical_features"] for e in data])
image_feats = np.array([e["image_features"] for e in data])
y_encoded = np.array([e["label_encoded"] for e in data])
...
X_train, X_test, y_train, y_test = train_test_split(
    X, y_encoded, test_size=0.2, shuffle=True, stratify=y_encoded, random_state=40
)
...
print(f"RBF SVM training time: {train_time:.4f} s")

test_pred = model.predict(X_test_s)
train_pred = model.predict(X_train_s)
print("Train accuracy:", model.score(X_train_s, y_train))
